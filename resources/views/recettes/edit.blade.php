            ...
            <div class="card-body">
                ...
                <form action="{{ route('recettes.update', $recette) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="description" rows="3" required>{{ old('description', $recette->description) }}</textarea>
                    </div>
                    ...
                    <div id="ingredientsList">
                        @php
                            $ingredientsToDisplay = old('ingredients', $recette->ingredients->map(function($ing) {
                                return [
                                    'nom' => $ing->nom,
                                    'quantite' => $ing->pivot->quantite,
                                    'unite' => $ing->pivot->unite,
                                    'nature' => $ing->nature ?? 'solide'
                                ];
                            })->toArray());
                        @endphp

                        @foreach($ingredientsToDisplay as $idx => $ing)
...
<script>
    let ingredientIndex = parseInt("{{ count($ingredientsToDisplay ?? []) }}", 10) || 0;
